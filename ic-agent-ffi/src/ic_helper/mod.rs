...
pub async fn query(
    ...
    let method_sig = get_method_signature(func_name, &ty_env, &actor)?;
    // 3. parse input arguments to blob
    let args_blb = blob_from_raw(func_args, &ty_env, &method_sig)?;
    // 4. get effective canister id
    let effective_canister_id =
        get_effective_canister_id(func_name, args_blb.as_slice(), canister_id)?;
    // 5. create agent
    let agent = create_agent(caller, IC_MAIN_NET)?;
    // 6. construct transaction then call it
    let rst_blb = agent
        .query(canister_id, func_name)
