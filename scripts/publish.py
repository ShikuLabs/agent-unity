...
@click.pass_context
def publish(ctx, release, dev, input, compress, version, output):
    mode = '--release' if release else ''
    devMode = '--dev' if dev else ''
    ...
    # pack
    cmd = f'{pack_script_path} {mode} {devMode} --input {input} --compress {compress} --version {version} --output {output}'
    stats = os.system(cmd)
    code = os.WEXITSTATUS(stats)
